...
def teach_node(state: GraphState) -> dict:
    ...
    logger.info("teach_node invoked | user_message=%r", user_message)
    ...
    logger.debug("teach_node profile | topic=%r concept=%r step=%d difficulty=%r", topic, concept, step, difficulty)
    ...
    try:
        ...
        logger.exception("teach_node agent invocation failed")
        raise

    # The result is an AIMessage (or similar)
    text = result.content if hasattr(result, "content") else str(result)

    logger.info("teach_node complete | step=%d->%d response_len=%d", step, step + 1, len(text))
